...
def proc_output_f1(output, target=['1', '2', 'SU4']):
    """
        Get only F1 scores.
    """
    start_pat = '1 ROUGE-{} Average'
...
def compute_rouge_lqsum(text_dp, ref_dp, split_sentences, max_len=250, verbose_output=False):
    """
        For project LQSum.
...
def compute_rouge_lqsum_per_file(text_dp, ref_dp, text_fn, split_sentences, max_len=250, verbose_output=False,
        target_pattern='#ID#_[\d]'):
    """
        For project LQSum.
    ...
    r = Rouge155(rouge_args=rouge_args)
    r.system_dir = text_dp
    r.system_filename_pattern = f'({text_fn})'

    r.model_dir = ref_dp
    r.model_filename_pattern = target_pattern
