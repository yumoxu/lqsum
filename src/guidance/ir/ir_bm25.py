...
def _rank(cid, query):
    res = tfidf_tools.build_rel_scores_bm25(cid, query)
...
def rank_e2e():
    ...
    :return:
    """
    rank_dp = join(path_parser.summary_rank, ir_config.IR_MODEL_NAME_BM25)
...
def ir_rank2records():
    ir_rec_dp = join(path_parser.summary_rank, ir_config.IR_RECORDS_DIR_NAME_BM25)
    ...
    for cid in tqdm(cids):
        retrieval_params = {
            'model_name': ir_config.IR_MODEL_NAME_BM25,  # fixme: this used TF in the first place
            'cid': cid,
