...
"""
    Module for IR with Weighted Count.
"""
def _rank(cid, query):
    res = tfidf_tools.build_rel_scores_weighted_count(cid, query)
...
def rank_e2e():
    rank_dp = join(path_parser.summary_rank, ir_config.IR_MODEL_NAME_WEIGHTED_COUNT)
...
def ir_rank2records():
    ir_rec_dp = join(path_parser.summary_rank, ir_config.IR_RECORDS_DIR_NAME_WEIGHTED_COUNT)
    ...
    for cid in tqdm(cids):
        retrieval_params = {
            'model_name': ir_config.IR_MODEL_NAME_WEIGHTED_COUNT,
            'cid': cid,
...
def tune():
    ...
    ir_tune_dp = join(path_parser.summary_rank, ir_config.IR_TUNE_DIR_NAME_WEIGHTED_COUNT)
    ir_tune_result_fp = join(path_parser.tune, ir_config.IR_TUNE_DIR_NAME_WEIGHTED_COUNT)
