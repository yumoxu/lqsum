...
def main():
    ...
    for ix, line in tqdm(enumerate(resfile), total=lcounter):
        ...
        probs = [p[1] for p in cline['class_probabilities']]
        bpe_ids = cline['bpe_ids']
        tags = [1 if p > opt.threshold else 0 for p in probs]
        # tags = [int(tag) for tag in cline['tags']]

        assert len(probs) == len(bpe_ids), f'{ix}th: len(probs): {len(probs)}, len(bpe_ids): {len(bpe_ids)}'
        ...
        if opt.tgt:
            yhat.append(tags)
            yprobs.append(probs)
            bpe_lines.append(bpe_ids)

        # if ix > 150:
        #     break
    if opt.tgt:
        print(f"Evaluating Model. #Records: {len(y)}")
        y_flat = []
        yhat_flat = []
        probs_flat = []
        for idx, (tgt, pred, pr, bpe_line) in enumerate(zip(y, yhat, yprobs, bpe_lines)):
            
            if len(pred) > len(tgt):
                print(f"---------------Inconsistent length: idx: {idx}------------------")
                print(f'pred: {len(pred)}, tgt: {len(tgt)}')
                print(f'pred: {pred}')
                print(f'tgt: {tgt}')
                assert len(tgt) > len(pred), f'more info -- len(bpe_line): {len(bpe_line)}'
                tgt = tgt[:len(pred)]
