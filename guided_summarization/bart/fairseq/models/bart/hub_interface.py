...
class BARTHubInterface(nn.Module):
    ...
    def encode_bped_sentence(self, bped_sentence: str) -> torch.LongTensor:
        """
    ...
    def sample(self, sentences: List[str], beam: int = 1, verbose: bool = False, 
        use_bped_input: bool = False, **kwargs) -> str:
        """
            yumo's revision: add param `use_bped_input`, to skip bpe encoding when using encoded input
        """
        # print(f'use_bped_input: {use_bped_input}')
        if not use_bped_input:
            input = [self.encode(sentence) for sentence in sentences]
        else:
            input = [self.encode_bped_sentence(sentence) for sentence in sentences]
