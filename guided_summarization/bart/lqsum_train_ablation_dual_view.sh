#!/bin/sh
# Ablation study for 64: no query focused view (we use standard enc-dec arch + a tagger)
# ARCH=guided_bart_large_with_tagging
# add: ARCH=guided_bart_large_with_tagging_and_std_enc_dec
ARCH=guided_bart_large_with_tagging_and_std_enc_dec
TAGGING_HEAD=binary_classification

LOG_DIR=/home/lqsum/log/model_72-bpeTags
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python train.py $DATA_BIN \
    ...
    --tag-on-doc \
    --load-z-tags \
    --mask-z-tokens-with-gumbel-scores \
    --fixed-tau 0.9 \
