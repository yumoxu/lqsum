...
def main(args):
    ...
    if args.tagdict:
        # tag_dict = task.load_dictionary(args.tagdict)
        tag_dict = task.load_tag_dictionary(tag_dict_fp=args.tagdict, 
            tag_one_as_mask=True, norm_for_special_token='0') 
        tag_dict.save(dict_path(lang='tag'))
    ...
    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1):
        # if args.dataset_impl == "raw" or lang == 'tag':
        if args.dataset_impl == "raw":
            # Copy original text file to destination folder, including tag files
